...
function bwWrapper(){
	...
	var pumpStatus_addr = 1022;
	var pdsh_Addr = 4010;
	var bw_trigAddr = 4004;
	var bwStatus_addr = 4005;

	//get duration from the PLC
	plc_client.readHoldingRegister(6514,1,function(resp){
		if (resp != undefined && resp != null){
			bwData2.duration = resp.register[0];	
		}  
		else{
			bwData2.duration = 3;
		}      
	...
	if (autoMan === 0){//auto mode
		bwData2.manBWcanRun = checkManualBW(bwData2.duration,Math.floor(now/100),dayToday);
	}
	else{//man Mode
		if(manPlay){// if playing manual shows, then block BW routine
			bwData2.manBWcanRun = 0;
		}
		else{//if playlist is not playing shows then we can run BW
			bwData2.manBWcanRun = 1;
		}
	}
	...
	plc_client.readCoils(pdsh_Addr,1,function(resp){
		if(resp.coils[0]){
			bwData2.PDSH_req4BW = 1;
		}
		else{
			bwData2.PDSH_req4BW = 0;
		}
	});

	if (bwData2.PDSH_req4BW){
		trigger_BW_PDSH = checkManualBW(bwData2.duration,Math.floor(now/100),dayToday);
	...
	//======================== BW Trigger conditions start ==============================
		...
		if (bwData2.SchBWStatus === 0) {
            ...
            var timeOffset = (alphaconverter.endtime(bwData2.schTime,1))*100;		
			if ( (dayToday === bwData2.schDay) && (( now >= (bwData2.schTime*100) ) && ( now <= timeOffset )) ){
				...
				if ( (bwData2.manBWcanRun == 1) && (playing == 0) ){
			...
			if ((bwData2.trigBacklog == 1) && (autoMan == 0)){
				var gapCheck = checkManualBW(bwData2.duration,Math.floor(now/100),dayToday);
				if (gapCheck){
					if (filtrationPump_fault === 0){
						watchDog.eventLog("Triggering backed-up scheduled BW now");
						//Issue the BW trigger to PLC
						trigBW(now,moment);
						bwData2.trigBacklog = 0;
					}
				}
			}
		...
		else if (bwData2.SchBWStatus === 2){// if blockSchBW = 2
            //watchDog.eventLog(alphaconverter.endtime(bwData2.timeLastBW,bwData2.timeout));
            var timeoutMoment = new Date(bwData2.blockBWuntil);
            if (moment.getTime() >= timeoutMoment.getTime()){
                bwData2.SchBWStatus = 0; //end of timeout
            } 
            else{
                bwData2.SchBWStatus = 2;
            }
            bwData2.timeoutCountdown = Math.round ( (timeoutMoment.getTime() - moment.getTime() )/1000 );
		}// end of else if

		else{
			//catch exception. bwData2.SchBWStatus should be 0, 1 or 2. 
		}

	//======================== BW Trigger conditions end ==============================
	//update txt file
	if (bwData2.blockBWuntil !== undefined){
		//watchDog.eventLog("bwData2 ok");
		fs.writeFileSync(homeD+'/UserFiles/backwash.txt',JSON.stringify(bwData2),'utf-8');
		fs.writeFileSync(homeD+'/UserFiles/backwashBkp.txt',JSON.stringify(bwData2),'utf-8');
	}
	else{
		//bwData2 is corrupt. Load default values and write to the file
		watchDog.eventLog("bwData2 Undefined. Pushed in default values.");
		bwData2 = {"BWshowNumber":999,"duration":3,"SchBWStatus":0,"timeout":86400,"timeoutCountdown":0,"timeLastBW":0,"trigBacklog":0,"manBWcanRun":1,"PDSH_req4BW":0,"schDay":1,"schTime":2300, "blockBWuntil":0 };
		fs.writeFileSync(homeD+'/UserFiles/backwash.txt',JSON.stringify(bwData2),'utf-8');
		fs.writeFileSync(homeD+'/UserFiles/backwashBkp.txt',JSON.stringify(bwData2),'utf-8');
	}
}
...
function trigBW(now,moment){
	...
	plc_client.writeSingleCoil(bw_trigAddr,1,function(resp){
		bwData2.SchBWStatus = 1;
		bwData2.timeLastBW = now;
		bwData2.blockBWuntil = new Date(moment.getTime() + (bwData2.timeout * 1000) );
		bwData2.trigBacklog = 0;
	});
}
