...
function initiate(arg){
    ...
    if(arg === 1){
        ...
        //watchDog.eventLog('Todays Date at 11:59:50: ' + tomorrowDate.getDate());
        tomorrowDate.setDate(tempdate.getDate() + 1);
        //watchDog.eventLog('Tomorrow Date: ' + tomorrowDate.getDate());
...
function tomorrowFirstShow(){
    var reqDate1;
    var reqMonth1;

    var tomorrowDate1 = new Date();
    var currentDate1 = new Date();
    tomorrowDate1.setDate(currentDate1.getDate() + 1);
    var current_day1 = tomorrowDate1.getDay();
    var current_date1 = tomorrowDate1.getDate();
    var current_month1 = tomorrowDate1.getMonth()+1;
    var current_year1 = tomorrowDate1.getYear();

    reqDate1 = current_date1;
    reqMonth1 = current_month1;

    var currentDaylistID1 = procure(reqDate1,reqMonth1, current_year1) + current_day1;

    var getSchedule1 = expand(extract(currentDaylistID1));

    var nextDayShowNumber = getSchedule1[0];
    var nextDayShowStarttime = getSchedule1[1];

    var nextDayShowData = [nextDayShowNumber,nextDayShowStarttime];
    return nextDayShowData;
}
    ...
    for(var k=1;k<arry.length;k+=2){
        ...
        if(typeof arry[k] === 'string'){
            ...
            if(arry[k].indexOf('p') > -1){
                ...
                arry[k] =  parseInt(arry[k]);
                //watchDog.eventLog("False Playlist Index");
...
function certify(playlistID,contents){
    ...
    function check(ID,set){
        var str1 = "p";
        var str2 = ID;
        var checkID = str1.concat(str2);
        watchDog.eventLog("CheckID " +checkID);
        for (var i = 1; i <= 699; i = i+2){
            if (schedule[i] === checkID){
                ...
                if(et>nextST){
                    ...
                    watchDog.eventLog("Next Show Start time is "+nextST+" New Playlist ENDTIME "+et);
                    var clashData = "Collision at Schedule" +set +" on " +day +" at" +nextST;
...
function checkInsert(st,sn,dayID){
    ...
    var localSch = [];
    localSch = expand(extract(alphabufferData[0]));
...
module.exports.tomorrowFirstShow=tomorrowFirstShow;
