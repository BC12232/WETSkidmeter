...
class CascadePumpDetailViewController: UIViewController,UIGestureRecognizerDelegate{
    ...
    private func readCurrentPumpSpeed() {
        ...
        if setFrequencyHandle.isMultipleTouchEnabled == true{
    ...
    private func checkForAutoManMode(response:[AnyObject]?){
        ...
        let startStopMode = Int(truncating: response![7] as! NSNumber)
            ...
            self.autoManualButton.isUserInteractionEnabled = true
    ...
    func changeAutManModeIndicatorRotation(autoMode:Bool){
        ...
        self.autoModeIndicator.rotate360Degrees(animate: autoMode)
...
//            self.setFrequencyHandle.transform = CGAffineTransform(scaleX: scale, y: scale);
